...
export default function RandomizePage({ params }: { params: Promise<{ roomCode: string }> }) {
    ...
    const [randomFoodResult, setRandomFoodResult] = useState<string | null>(null);
    const [restaurants, setRestaurants] = useState<any[]>([]);
    const [randomizingError, setRandomizingError] = useState<string | null>(null);
    const [randomizationComplete, setRandomizationComplete] = useState(false);

    // state สำหรับการแสดงร้านอาหารเพิ่มเติม
    const [showAllRestaurants, setShowAllRestaurants] = useState(false);
    const [selectedRestaurant, setSelectedRestaurant] = useState<number | null>(null);

    // state สำหรับเอฟเฟกต์การสุ่มแบบ CSGO ที่ปรับปรุง
    const [foodOptions, setFoodOptions] = useState<string[]>([]);
    const [displayedFoods, setDisplayedFoods] = useState<string[]>([]);
    const [animationActive, setAnimationActive] = useState(false);
    const [animationSpeed, setAnimationSpeed] = useState(50);
    const [animationPhase, setAnimationPhase] = useState(1); // 1: เริ่มต้นเร็ว, 2: ช้าลง, 3: จบ
    const [spinEffect, setSpinEffect] = useState(0); // สำหรับเอฟเฟกต์การสั่น
    ...
    const areAllMembersReady = members.length > 0 &&
        members.every(member => readyStatus[member] === true);
    ...
    const startRandomize = async () => {
        try {
            ...
            const spinLength = 120; // เพิ่มความยาวของการสุ่มให้มากขึ้น
            const centerIndex = 15; // ตำแหน่งตรงกลาง

            // สร้างรายการที่จะหมุน โดยใส่ชื่ออาหารจริงไว้ตรงกลางตอนสุดท้าย
            ...
            const phase1Length = 50;

            // เฟส 2: ช้าลงเรื่อยๆ
            const phase2Speed = 60; // ms
            const phase2Length = 40;

            // เฟส 3: ช้ามากและค่อยๆ หยุด
            const phase3Speed = 100; // ms
            const phase3Length = 30;
                    ...
                    const phase2Interval = setInterval(() => {
                        ...
                        if (index >= phase1Length + phase2Length) {
                            ...
                            const phase3Interval = setInterval(() => {
                                ...
                                // ลดเอฟเฟกต์สั่น
                                setSpinEffect(Math.random() * 1 - 0.5);
    ...
    return (
        // background-color
        <div
            className="fixed inset-0 flex items-center justify-center bg-orange-100 overflow-auto py-8"
        >

            <div className="bg-white p-8 rounded-2xl shadow-2xl text-center max-w-lg w-full relative z-10 mx-4">
                {/* แสดงคอนเฟตติเมื่อการสุ่มเสร็จสิ้น */}
                {showConfetti && (
                    <div className="absolute inset-0 overflow-hidden pointer-events-none z-50">
                        <div className="confetti-container">
                            {Array.from({ length: 50 }).map((_, i) => {
                                ...
                                const color = ['#FFC107', '#FF5722', '#4CAF50', '#2196F3', '#9C27B0'][Math.floor(Math.random() * 5)];
                ...
                {/* หัวข้อและข้อมูลห้อง */}
                <div className="mb-6">
                    <h1 className="text-3xl font-bold text-orange-600 mb-2">สุ่มอาหาร</h1>
                    <div className="bg-orange-50 rounded-xl px-4 py-2 inline-flex items-center">
                        <span className="text-orange-600 font-medium">ห้อง:</span>
                        <span className="ml-2 bg-white px-3 py-1 rounded-lg shadow-sm text-orange-700 font-bold">{roomCode}</span>
                    </div>
                </div>

                <div className="mt-4 text-center">
                    <h2 className="font-semibold text-xl mb-6 text-gray-700">สุ่มตัดสินใจว่าวันนี้จะกินอะไรดี</h2>

                    {!randomizationComplete && (
                        <div className="mb-6">
                            {isRandomizing || animationActive ? (
                                <div className="mb-6">
                                    <div className="bg-gradient-to-b from-yellow-50 to-orange-50 p-6 rounded-xl shadow-lg border border-orange-200 relative">
                                        {/*<div className="text-xl font-bold text-orange-600 mb-4 relative z-10">*/}
                                        {/*    {animationPhase === 1 && "กำลังค้นหา..."}*/}
                                        {/*    {animationPhase === 2 && "ใกล้ได้คำตอบแล้ว..."}*/}
                                        {/*    {animationPhase === 3 && "เกือบได้แล้ว..."}*/}
                                        {/*</div>*/}

                                        {/* คำแนะนำ */}
                                        {/*<div className="absolute top-2 right-2 bg-white bg-opacity-80 px-2 py-1 rounded-full text-xs text-gray-500 shadow-sm">*/}
                                        {/*    จะกินอะไรดีนะ?*/}
                                        {/*</div>*/}

                                        {/* เอฟเฟกต์การสุ่มแบบปรับปรุง */}
                                        <div
                                            className={`relative overflow-hidden h-24 rounded-xl ${animationPhase === 3 ? 'border-2' : 'border'} ${
                                        ...
                                        >
                                            ...
                                            {/*/!* กรอบเน้นตำแหน่งตรงกลาง *!/*/}
                                            {/*<div*/}
                                            {/*    className="absolute inset-y-0 left-1/2 border-l-2 border-r-2 border-orange-500 transform -translate-x-1/2 z-10 transition-all duration-300"*/}
                                            {/*    style={{*/}
                                            {/*        width: `${32 + glowIntensity * 8}px`,*/}
                                            {/*        opacity: 0.3 + glowIntensity * 0.5,*/}
                                            {/*        boxShadow: `0 0 ${10 + glowIntensity * 20}px rgba(255, 172, 28, ${0.1 + glowIntensity * 0.4})`*/}
                                            {/*    }}*/}
                                            {/*></div>*/}
                                        ...
                                        {/*<div className="text-gray-500 mt-2 text-sm italic">*/}
                                        {/*    {animationPhase === 1 && "กำลังค้นหาตัวเลือกที่ดีที่สุด..."}*/}
                                        {/*    {animationPhase === 2 && "เหลืออีกไม่นาน..."}*/}
                                        {/*    {animationPhase === 3 && "รอแป๊บนึงเท่านั้น..."}*/}
                                        {/*</div>*/}
                                    </div>
                                </div>
                            ) : (
                                <button
                                    ...
                                    className={`w-full py-6 px-6 rounded-xl text-lg font-medium transition-all ${
                                        isRandomizing
                                ...
                                >
                                    <span className="flex items-center justify-center">
                                      <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24"
                                           ...
                                           strokeLinejoin="round" className="mr-2 animate-pulse">
                    ...
                    {randomizingError && (
                        <div className="bg-red-100 text-red-800 p-3 rounded-lg mb-4">
                            ...
                            <button
                                onClick={startRandomize}
                                className="mt-2 w-full bg-red-500 text-white p-2 rounded hover:bg-red-600 transition-all"
                            >
                                ลองใหม่อีกครั้ง
                            </button>
                        </div>
                    ...
                    {randomFoodResult && randomizationComplete && (
                        <div className="mt-4">
                            <div className="text-center mb-2">
                                <h3 className="text-xl font-semibold">ผลการสุ่ม! 🎉</h3>
                            </div>

                            {/* แสดงผลลัพธ์และร้านอาหารแยกส่วนชัดเจน */}
                            <div className="grid grid-cols-2 gap-4 mb-5">
                                {/* คอลัมน์ซ้าย - ผลลัพธ์ประเภทอาหาร */}
                                <div className="col-span-1">
                                    <div className="bg-gradient-to-br from-orange-500 to-orange-600 p-6 rounded-xl shadow-lg text-white flex flex-col items-center justify-center h-full min-h-56 transform transition-all hover:scale-105 hover:shadow-xl">
                                        <div className="text-sm uppercase tracking-wider mb-2 opacity-90">สุ่มได้ประเภทอาหาร</div>
                                        <div className="text-4xl font-bold my-4 drop-shadow-md">
                                            {randomFoodResult}
                                        </div>
                                ...
                                <div className="col-span-1">
                                    <div className="bg-white rounded-xl shadow-lg border border-gray-100 h-full flex flex-col overflow-hidden transform transition-all hover:shadow-xl">
                                                ...
                                                {restaurants[0].imageUrl && restaurants[0].imageUrl !== "No image available" ? (
                                                    <div className="flex-grow p-2">
                                                        <img
                                                            ...
                                                            className="w-full h-40 object-cover rounded-lg shadow-md hover:opacity-95 transition-opacity"
                                                            onError={(e) => {
                            ...
                            {restaurants && restaurants.length > 1 && (
                                <div className="mt-4">
                                    <button
                                        ...
                                        className="flex items-center justify-center mx-auto bg-blue-50 hover:bg-blue-100 text-blue-600 font-medium py-2 px-4 rounded-full mb-2 transition-all"
                                    >
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="mr-1" viewBox="0 0 16 16">
                                            {showAllRestaurants ?
                                                        ...
                                                        <div className="mt-3 animate-fadeIn">
                                                            {restaurant.imageUrl && restaurant.imageUrl !== "No image available" ? (
                                                                <img
                                                                    ...
                                                                    alt={restaurant.name}
                                                                    className="w-full h-32 object-cover rounded-lg shadow-md mt-2 hover:opacity-95 transition-opacity"
                                                                    onError={(e) => {
                                                                        e.currentTarget.onerror = null;
                                                                        e.currentTarget.src = "https://placehold.co/400x300/orange/white?text=ไม่พบรูปภาพ";
                                                                    }}
